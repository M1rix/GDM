...
export class ProfileService {
  ...
  protected convertDateFromClient(profile: IProfile): IProfile {
    return Object.assign({}, profile, {
      createdDate: profile.createdDate?.isValid() ? profile.createdDate.toJSON() : undefined,
      lastModifiedDate: profile.lastModifiedDate?.isValid() ? profile.lastModifiedDate.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((profile: IProfile) => {
        ...
        profile.lastModifiedDate = profile.lastModifiedDate ? dayjs(profile.lastModifiedDate) : undefined;
      });
    }
    return res;
  }
